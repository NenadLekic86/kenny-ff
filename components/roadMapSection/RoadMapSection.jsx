...
const RoadMapSection = () => {
  ...
  const transition = { type: 'spring', stiffness: 50, damping: 40 };
  const x = useTransform(scrollY, [0, 0], [0, -130], { clamp: false });

  useEffect(() => {
     if (isInView) {
      controls.start({ x: x.get(), transition });
    } else {
      controls.start({ x: 0, transition });
    }
  }, [isInView, controls, x, transition]);
        ...
        <div className="relative w-full h-2.5 rounded-full bg-gradient-to-r from-[#E7DBED] via-[#B996CA_97%] to-[#B996CA] mb-10">
          ...
          >
            <Image 
              src={Icon}
              alt=''
              className='relative max-w-[34px] top-[-21px] left-[16px] md:max-w-full md:top-[-52px] md:left-[128px]'
            />
          </motion.div>
        </div>
          ...
          >
            <SwiperSlide>
              ...
              <div className='roadMap-card-box'>
                {/* Dot on progress bar */}
                <div className="absolute top-[-53px] md:top-[-62px] left-0 md:left-[15px] transform -translate-y-0 -translate-x-1/2 w-4 h-4 md:w-8 md:h-8 bg-white rounded-full flex items-center justify-center">
