...
const CountdownTimer = () => {
  ...
  const getDigitsArray = (number, digitCount) => {
    ...
    return paddedNumber.split('').map((digit, index) => (
      <span key={index} className="timer-digit text-[#332010] text-[20px] font-medium rounded-[20px] bg-white px-5 py-4 mr-2 w-[49px]">
        {digit}
      </span>
    ));
  };
  ...
  return (
    <div className='myTimerContainer mt-8 mb-20'>
      <div className='flex items-center justify-center'>
        {/* Hours */}
        {getDigitsArray(hours, 2)}
        <span className="timer-separator m-4">
